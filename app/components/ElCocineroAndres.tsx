...
const COMBOS: Combo[] = [
  { id: 'combo-2', name: '2 Tacos', price: 5.5, desc: '1 Birria + 1 Pollo Copal' },
  { id: 'combo-3', name: '3 Tacos', price: 8, desc: '1 Birria + 2 Pollo Copal' },
  { id: 'combo-4', name: '4 Tacos', price: 10, desc: '2 Birria + 2 Pollo Copal' },
];
...
export default function ElCocineroAndres({ initialReviews }: ElCocineroAndresProps) {
  ...
  const whatsappNumber = '507 6000-0000';
  ...
  const price = (p: number) => '$' + (p % 1 ? p.toFixed(2).replace(/0$/, '') : p);

  const wa = (name: string) => {
    const num = String(whatsappNumber).replace(/\D/g, '');
    return 'https://wa.me/' + num + '?text=' + encodeURIComponent('¡Hola El Cocinero Andrés! Quiero pedir: ' + name);
  };
  ...
  const items = DISHES.map((d) => {
    ...
    return {
      ...
      ratingLabel: avg ? avg.toFixed(1) + ' · ' + n + ' review' + (n > 1 ? 's' : '') : 'Sin reviews aún',
    };
  });
  ...
  const nameOf = (id: string) => (DISHES.find((d) => d.id === id) || { name: id }).name;

  const featured = ['taco-birria', 'ceviche-330', 'mini-pizzas']
    .map((id) => items.find((i) => i.id === id))
    .filter((f) => f !== undefined);

  const homeReviews = reviews.slice(0, 2).map((r) => ({ ...r, starsStr: starStr(r.stars) }));
      ...
      {tab === 'inicio' && (
        <main className={styles.main}>
          <section className={styles.heroSection}>
            ...
            <h1 className={styles.heroTitle}>Sabor que se apodera de tus sentidos</h1>
            <p className={styles.heroDesc}>
              En esta edición, cada platillo está diseñado para ser saludable y bajo en calorías — sin sacrificar ni un gramo de sabor.
            </p>
          </section>
          ...
          <section className={styles.chefSection}>
            ...
            <div>
              ...
              <p className={styles.chefDesc}>
                Un apasionado por la cocina y por complacer a sus comensales. Te llevamos a una experiencia sensorial en cada bocado.
              </p>
            </div>
          </section>
            ...
            <div className={styles.promosList}>
              <div className={styles.promoCard}>
                <div>
                  ...
                  <div className={styles.promoDesc}>2 Birria + 2 Pollo Copal · llévalo por menos</div>
                </div>
              ...
              <div className={styles.promoPlaceholder}>
                ...
                <div className={styles.placeholderDesc}>Espacio para promoción especial — edítame</div>
              </div>
            </div>
          </section>
                ...
                <div key={f.id} className={styles.favoriteCard}>
                  ...
                  <div className={styles.favoriteContent}>
                    ...
                    <div className={styles.favoritePrice}>
                      <span className={styles.favoritePriceValue}>{f.priceLabel}</span>
          ...
          <section className={styles.reviewsPreviewSection}>
            <div className={styles.reviewsHeader}>
              ...
              <a onClick={() => setTab('reviews')} className={styles.reviewsLink}>
                Ver todos
              </a>
            </div>
              ...
              <article key={d.id} className={styles.dishCard}>
                ...
                <div className={styles.dishContent}>
                  ...
                  <div className={styles.dishRating}>
                    ...
                    <span className={styles.dishRatingLabel}>{d.ratingLabel}</span>
                  </div>
                  <p className={styles.dishDesc}>{d.desc}</p>
                  <a href={d.wa} target="_blank" rel="noopener noreferrer" className={styles.whatsappButton}>
            ...
            {combos.map((c) => (
              <div key={c.id} className={styles.comboCard}>
                ...
                <div className={styles.comboRight}>
                  ...
                  <a href={c.wa} target="_blank" rel="noopener noreferrer" className={styles.comboButton}>
                    Pedir
                  </a>
                </div>
              </div>
        ...
        <main className={styles.main}>
          ...
          <div className={styles.ratingCard}>
            ...
            <div>
              ...
              <div className={styles.ratingLabel}>{reviews.length} reviews de nuestros comensales</div>
            </div>
          </div>
          ...
          <div className={styles.formCard}>
            ...
            <div className={styles.formFields}>
              ...
              />
              <select value={fDish} onChange={(e) => setFDish(e.target.value)} className={styles.formSelect}>
                {DISHES.map((d) => (
              ...
              />
              <div className={styles.formPhotoPlaceholder}>📷 Adjuntar foto (opcional) — demo</div>
              <button onClick={submitReview} disabled={submitting || cooldown > 0} className={styles.formButton}>
                Publicar review
              </button>
